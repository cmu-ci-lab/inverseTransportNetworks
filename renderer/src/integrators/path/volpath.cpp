...
public:
	...
	Spectrum_AD Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
		...
		while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
			...
			if (rRec.medium && rRec.medium->sampleDistance(Ray(ray, 0, its.t), mRec, rRec.sampler)) {
				...
				*/
				...
				throughput *= mRec.sigmaS * mRec.transmittance * (Float(1.0f) / mRec.pdfSuccess);
				...
				if (rRec.type & RadianceQueryRecord::EDirectMediumRadiance) {
					...
					Spectrum_AD value = scene->sampleAttenuatedEmitterDirect(
...
//					if (!value.isZero()) {
						...
						Spectrum_AD phaseVal = phase->eval(pRec);

						// Removing this check, because it affects autodiff computation.
//						if (phaseVal != 0) {
							/* Calculate prob. of having sampled that direction using
							   phase function sampling */
							// Float phasePdf = (emitter != NULL && emitter->isOnSurface() && dRec.measure == ESolidAngle)
							...
							Li += weight * phaseVal * throughput * value;
...
//					}
				...
				Spectrum_AD phaseVal = phase->sampleAD(pRec, phasePdf, rRec.sampler);
...
//				if (!value.isZero() && (rRec.type & RadianceQueryRecord::EDirectMediumRadiance)) {
				if (rRec.type & RadianceQueryRecord::EDirectMediumRadiance) {
					...
					Li += miWeight(phasePdf, emitterPdf)*throughput * value;
				}
				...
				*/
				...
				if (!its.isValid()) {
					...
					if ((rRec.type & RadianceQueryRecord::EEmittedRadiance)
						...
						Spectrum_AD value = scene->evalEnvironment(ray) * throughput;
						if (rRec.medium) {
							Spectrum_AD tempTransmittance = rRec.medium->evalTransmittance(ray, rRec.sampler);

							value *= tempTransmittance;
						}
						Li += value;
					}
...
//													woDotGeoN * Frame::cosTheta(bRec.wo) > 0)) {
						if (!m_strictNormals || woDotGeoN * Frame::cosTheta(bRec.wo) > 0) {
							/* Calculate prob. of having generated that direction
							   using BSDF sampling */
							Float bsdfPdf = (emitter != NULL && emitter->isOnSurface()
									&& dRec.measure == ESolidAngle)
									? bsdf->pdf(bRec) : (Float) 0.0f;

							/* Weight using the power heuristic */
							const Float weight = miWeight(dRec.pdf, bsdfPdf);
							Li += weight * bsdfVal * throughput * value;
						}
//					}
				}
...
//				if (!value.isZero() && (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance)) {
				if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance) {
					...
					Li += miWeight(bsdfPdf, emitterPdf) * throughput * value;
				}
...
//				FloatAD q = getMin(throughput.max() * eta * eta, (Float) 0.95f);
//				if (rRec.nextSample1D() >= q.val())
//					break;
				Float q = std::min(Float(throughput[0].val()) * eta * eta, (Float) 0.95f);
	...
	void rayIntersectAndLookForEmitter(const Scene *scene, Sampler *sampler,
			...
			DirectSamplingRecord &dRec, Spectrum_AD &value) const {
		Intersection its2, *its = &_its;
		Spectrum_AD transmittance(1.0f);
		bool surface = false;
		int interactions = 0;
...
//				return;
		...
		if (surface) {
			...
			if (its->isEmitter()) {
				...
				value = its->Le(-ray.d) * transmittance;
			}
		} else {
			...
			if (env && env->fillDirectSamplingRecord(dRec, ray))
				value = env->evalEnvironment(RayDifferential(ray)) * transmittance;
		}
	}
	...
	stan::math::var test(Float x) const {
