...
/*  in mitsuba/render/BitmapAD.h. Ideally we'd like to have optimized code    */
...
template <typename T> struct FormatConverterImpl : public FormatConverterAD {
	...
	virtual Conversion getConversion() const {
		return std::make_pair(
			(BitmapAD::EComponentFormat) detail::get_pixelformat<SourceFormat>::value,
			(BitmapAD::EComponentFormat) detail::get_pixelformat<DestFormat>::value
		);
	}
	...
	virtual void convert(
			...
			size_t count, Float multiplier, Spectrum::EConversionIntent intent, int channelCount, int nDerivative) const {
		...
		if ((int) detail::get_pixelformat<SourceFormat>::value == (int) detail::get_pixelformat<DestFormat>::value &&
			...
			switch (sourceFormat) {
				case BitmapAD::ELuminance:            channelCount = 1; break;
				case BitmapAD::EDerivative:           channelCount = nDerivative + 1; break;
				case BitmapAD::ELuminanceAlpha:       channelCount = 2; break;
				case BitmapAD::ERGB:
				case BitmapAD::EXYZ:                  channelCount = 3; break;
				case BitmapAD::EXYZA:
				case BitmapAD::ERGBA:                 channelCount = 4; break;
				case BitmapAD::ESpectrum:             channelCount = SPECTRUM_SAMPLES + nDerivative; break;
				case BitmapAD::ESpectrumAlpha:        channelCount = SPECTRUM_SAMPLES + 1; break;
				case BitmapAD::ESpectrumAlphaWeight:  channelCount = SPECTRUM_SAMPLES + nDerivative + 2; break;
				case BitmapAD::EMultiChannel:         break;
				default:
		...
		std::vector<Float> specVec = std::vector<Float>(nDerivative + 1);	
		...
		switch (sourceFormat) {
			case BitmapAD::ELuminance: {
					switch (destFormat) {
						...
						case BitmapAD::ESpectrumAlphaWeight:
							for (size_t i=0; i<count; ++i) {
								DestFormat value = convertScalar<DestFormat>(*source++, sourceGamma, precomp, multiplier, invDestGamma);
								for (int j=0; j<SPECTRUM_SAMPLES; ++j)
									*dest++ = value;
			...
			case BitmapAD::ESpectrum: {
					switch (destFormat) {
						case BitmapAD::ELuminance:
							...
							break;
						case BitmapAD::EDerivative:
							for (size_t i=0; i<count; ++i) {
								for (int j=0; j<SPECTRUM_SAMPLES + nDerivative; ++j) {
									specVec[j] = convertScalar<Float>(*source++, sourceGamma);
								}
								for (int j=0; j<SPECTRUM_SAMPLES + nDerivative; ++j) {
									*dest++ = convertScalar<DestFormat>(specVec[j], 1.0f, NULL, multiplier, invDestGamma);
								}
							}
							break;	
						case BitmapAD::ELuminanceAlpha:
						...
						case BitmapAD::EDerivative:
							for (size_t i=0; i<count; ++i) {
								...
								for (int j=0; j<SPECTRUM_SAMPLES + nDerivative; ++j) {
									...
									*dest++ = convertScalar<DestFormat>(specVec[j]*invWeight, 1.0f, NULL, multiplier, invDestGamma);
								}
							}
							break;		
						case BitmapAD::ELuminanceAlpha:
...
public:
	inline RegisterConverter(FormatConverterAD::ConverterMap &map) : m_converters(map) { }

	template <class T> void operator() (T) const {
		FormatConverterAD *converter = new T();
		FormatConverterAD::Conversion cType = converter->getConversion();
		m_converters[cType] = converter;
	}
private:
	FormatConverterAD::ConverterMap &m_converters;
};

FormatConverterAD::ConverterMap FormatConverterAD::m_converters;

void FormatConverterAD::staticInitialization() {
	mpl::for_each<ConverterImplementations>(RegisterConverter(m_converters));
}

void FormatConverterAD::staticShutdown() {
...
const FormatConverterAD *FormatConverterAD::getInstance(Conversion types) {
