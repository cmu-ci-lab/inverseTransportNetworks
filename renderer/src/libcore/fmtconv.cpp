...
namespace detail {
	...
	template <> struct get_pixelformat<FloatAD> {
		enum { value = Bitmap::EFloatAD };
	};
	template <> struct get_pixelformat<FloatAD_shared> {
		enum { value = Bitmap::EFloatAD_shared };
	};	
...
template <typename T> struct FormatConverterImpl : public FormatConverter {
	...
	struct format_traits
	{
		...
		static const bool is_float =
			...
			boost::is_same<FormatType, FloatAD>::value ||
			boost::is_same<FormatType, FloatAD_shared>::value;
	...
	virtual void convert(
		...
		if ((int) detail::get_pixelformat<SourceFormat>::value == (int) detail::get_pixelformat<DestFormat>::value &&
			...
			switch (sourceFormat) {
				...
				case Bitmap::EDerivative:           channelCount = nDerivative + 1; break;
				...
				case Bitmap::ESpectrum:             channelCount = SPECTRUM_SAMPLES + nDerivative; break;
				case Bitmap::ESpectrumAlpha:        channelCount = SPECTRUM_SAMPLES + 1; break;
				case Bitmap::ESpectrumAlphaWeight:  channelCount = SPECTRUM_SAMPLES + nDerivative + 2; break;
...
typedef mpl::vector<
	uint8_t, uint16_t, uint32_t, half, float, double,FloatAD, FloatAD_shared> SupportedTypes;
