...
public:
	HDRFilm(const Properties &props) : Film(props) {
		...
		int nDerivative = props.m_theta_FloatAD.size() + props.m_theta.size() + props.m_theta_TextureAD.size();
		...
		for (size_t i=0; i<pixelFormats.size(); ++i) {
			...
			if (pixelFormat == "derivative") {
				m_pixelFormats.push_back(Bitmap::EDerivative);
				m_channelNames.push_back(name + "Forward");				
				for (int nIDSpectrum_AD = 0; nIDSpectrum_AD < props.id_AD_Spectrum_AD.size(); nIDSpectrum_AD++) {
					m_channelNames.push_back(name + props.id_AD_Spectrum_AD[nIDSpectrum_AD]);
				}
				for (int nIDFloatAD = 0; nIDFloatAD < props.id_AD_FloatAD.size(); nIDFloatAD++) {
					m_channelNames.push_back(name + props.id_AD_FloatAD[nIDFloatAD]);
				}
				for (int nIDTextureAD = 0; nIDTextureAD < props.id_AD_TextureAD.size(); nIDTextureAD++) {
					m_channelNames.push_back(name + props.id_AD_TextureAD[nIDTextureAD]);
				}
		...
		if (m_pixelFormats.size() == 1) {
			m_storage = new ImageBlock(nDerivative, Bitmap::ESpectrumAlphaWeight, m_cropSize);

		} else {
			m_storage = new ImageBlock(nDerivative, Bitmap::EMultiSpectrumAlphaWeight, m_cropSize,
