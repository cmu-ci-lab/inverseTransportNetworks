...
public:
	...
	ref<WorkResult> createWorkResult() const {
		return new ImageBlock(m_nDerivatives, m_pixelFormat,
	...
	void prepare() {
		...
		m_nDerivatives = m_integrator->m_theta_integrator.size() + m_integrator->m_theta_integrator_FloatAD.size() + m_integrator->m_theta_integrator_TextureAD.size();
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult,
		...
		m_integrator->renderBlock(m_scene, m_sensor, m_sampler,
			block, stop, m_hilbertCurve.getPoints(),counter);
#ifdef MTS_DEBUG_FP
		disableFPExceptions();
#endif
	}
