...
SceneHandler::SceneHandler(const ParameterMap &params,
	...
	m_tags["textureAD"]  = TagEntry(ETextureAD,    MTS_CLASS(TextureAD));	
	...
	m_tags["floatAD"]    = TagEntry(EFloatAD,    (Class *) NULL);	
	...
	m_tags["vector_AD"]  = TagEntry(EVector_AD,  (Class *) NULL);	
	...
	m_tags["spectrum_AD"]= TagEntry(ESpectrum_AD,(Class *) NULL);
...
void SceneHandler::endElement(const XMLCh* const xmlName) {
	...
	switch (tag.first) {
		...
		case EFloatAD: {
				Float value = parseFloat(name, context.attributes["value"]);
				FloatAD_shared valueAD = FloatAD_shared(value);
				temp_m_theta_FloatAD.push_back(valueAD);		
				context.parent->properties.m_theta_FloatAD = temp_m_theta_FloatAD;
				temp_id_AD_FloatAD.push_back(context.attributes["id_AD"]);
				context.parent->properties.id_AD_FloatAD = temp_id_AD_FloatAD;
			}
			break;
		case EBoolean: {
		...
		case EString: {
			...
			if (context.attributes["flag"] == "1") {
				context.parent->properties.m_theta_string.push_back(context.attributes["value"]);
				std::vector<std::string> weights = tokenize(context.attributes["value"]," ,;");
				std::vector<FloatAD_shared> weightsAD = std::vector<FloatAD_shared>(weights.size());
				char *end_ptr = NULL;
				int num = 1;
				std::string curW = "w" + std::to_string(num);
				for (size_t i=0; i<weights.size(); ++i) {
					temp_id_AD_FloatAD.push_back(curW);
					Float weight = (Float) strtod(weights[i].c_str(), &end_ptr);
					
					temp_m_theta_FloatAD.push_back(FloatAD_shared(weight));
					num = num + 1;
					curW = "w" + std::to_string(num);	
				}
				context.parent->properties.id_AD_FloatAD = temp_id_AD_FloatAD;
				context.parent->properties.m_theta_FloatAD = temp_m_theta_FloatAD;	
			} else {
		...
		case EVector_AD: {
				int num = 1;
				std::string curW = "w" + std::to_string(num);
				while (!context.attributes[curW].empty()) {
					temp_m_theta_FloatAD.push_back(FloatAD_shared(parseFloat(name, context.attributes[curW])));
					temp_id_AD_FloatAD.push_back(curW);
					num = num + 1;
					curW = "w" + std::to_string(num);
				}
		...
		case ESpectrum_AD: {
						...
						bool hasFilename = context.attributes.find("filename") != context.attributes.end();
						bool hasIntent = context.attributes.find("intent") != context.attributes.end();
						bool hasFlag = context.attributes.find("flag") != context.attributes.end();
						...
						if (hasValue == hasFilename) {
							...
							if (tokens.size() == 1 && tokens[0].find(':') == std::string::npos) {
								...
								value[0] = parseFloat(name, tokens[0]);
								Spectrum_shared spec;
								if (intent == Spectrum::EReflectance) {
									
									spec = Spectrum_shared(value[0]);
								} else {
									spec = Spectrum_shared((Spectrum::getD65() * value[0])[0]);
									
								}
								temp_m_theta.push_back(spec);	
								context.parent->properties.m_theta = temp_m_theta;
								temp_id_AD_Spectrum_AD.push_back(context.attributes["id_AD"]);
								context.parent->properties.id_AD_Spectrum_AD = temp_id_AD_Spectrum_AD;
							} else {
								if (hasIntent)
									XMLLog(EError, "<spectrum>: 'intent' can only be specified when given a single-valued argument.");
								if (tokens[0].find(':') != std::string::npos) {
		...
		default: {
				...
				if (tag.second == MTS_CLASS(Shape)
					...
					try {	
						props.m_theta = temp_m_theta;
						props.m_theta_FloatAD = temp_m_theta_FloatAD;		
						props.id_AD_Spectrum_AD = temp_id_AD_Spectrum_AD;	
		...
		if (object) {
			...
			for (std::vector<std::pair<std::string, ConfigurableObject *> >
				...
				if (it->second != NULL) {
					...
					it->second->setParent(object);
					it->second->decRef();
					Properties &propsnew = it->second->getPropertiesAD();
					if (propsnew.m_theta_TextureAD.size() != 0) {
						temp_TextureAD = propsnew.m_theta_TextureAD;
						temp_id_TextureAD = propsnew.id_AD_TextureAD;
					}
