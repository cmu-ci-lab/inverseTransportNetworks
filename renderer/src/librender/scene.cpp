...
// void Scene::setVector(float x){
// 	scene->g[0] = x;
// }
void Scene::setTestvar(Spectrum_AD v) {
	testVar = v;
}
...
void Scene::initializeBidirectional() {
	...
	for (ref_vector<Emitter>::iterator it = m_emitters.begin();
		...
		if (!emitter->isEnvironmentEmitter()) {
		...
		} else if (emitter->isEnvironmentEmitter() && emitter->isRegularEnv()) {
			...
			std::vector<ref<Shape>> shapes = emitter->createShapes(this);

			for (int i = 0; i < shapes.size(); ++i) {
				if (shapes[i] != NULL)
					m_specialShapes.push_back(shapes[i]);

				aabb.expandBy((emitter->getAABBs())[i]);
				if (!(emitter->getType() & Emitter::EDeltaPosition))
					m_degenerateEmitters = false;
			}
...
Spectrum_AD Scene::evalTransmittance(const Point &p1, bool p1OnSurface, const Point &p2, bool p2OnSurface,
...
Spectrum_AD Scene::evalTransmittanceAll(const Point &p1, bool p1OnSurface, const Point &p2, bool p2OnSurface,
		Float time, const Medium *medium, int &interactions, Sampler *sampler) const {
	Vector d = p2 - p1;
	Float remaining = d.length();
	d /= remaining;
...
Spectrum_AD Scene::sampleEmitterDirect(DirectSamplingRecord &dRec,
	...
	Spectrum_AD value;
	if (emitter->isRegularEnv()) {
		value = Spectrum_AD((emitter->sampleDirect(dRec, sample))[0]);
	} else {
		value = emitter->sampleDirectAD(dRec, sample);
	}
...
Spectrum_AD Scene::sampleAttenuatedEmitterDirect(DirectSamplingRecord &dRec,
...
Spectrum_AD Scene::sampleSensorDirect(DirectSamplingRecord &dRec,
		const Point2 &sample, bool testVisibility) const {
	Spectrum_AD value = Spectrum_AD(m_sensor->sampleDirect(dRec, sample));
...
Spectrum_AD Scene::sampleAttenuatedSensorDirect(DirectSamplingRecord &dRec,
...
Float Scene::pdfEmitterDirect(const DirectSamplingRecord &dRec) const {
	...
	const Emitter *emitter = m_emitters[0].get();
...
Spectrum Scene::sampleEmitterPosition(
	...
	Point2 sample(_sample);
	cout << "called sampleEmitterPosition" << endl;
	/* Randomly pick an emitter */
	Float emPdf;
	size_t index = m_emitterPDF.sampleReuse(sample.x, emPdf);
	const Emitter *emitter = m_emitters[index].get();
