...
public:
	...
	void configure() {
		...
		m_components.clear();
		m_components.push_back(m_filename.filename().string());
	...
	Spectrum eval(const Intersection &its, const Vector &d) const {
		return Spectrum(evalEnvironment(RayDifferential(its.p, -d, its.time))[0].val());
	}
	...
	Spectrum_AD evalEnvironment(const RayDifferential &ray) const {
		...
		return Spectrum_AD(value[0]) * m_scale;
	}
	...
	Float pdfDirection(const DirectionSamplingRecord &dRec,
		...
		const Transform &trafo = m_worldTransform->eval(pRec.time);
		Spectrum value;
		return internalPdfDirection(-trafo.inverse()(dRec.d),value);
	}
...
//// sampledirect is called
	...
	// evaluate spectrum given a fixed direction
	Spectrum evalDirect(const DirectSamplingRecord &dRec, Float &pdf) const {
		const Transform &trafo = m_worldTransform->eval(dRec.time);
		
		/* Sample a direction from the environment map */

		Ray ray(dRec.ref, trafo(dRec.d), 0);

		Float nearT, farT;

		Spectrum value;
		pdf = internalPdfDirection(trafo.inverse()(dRec.d),value);
	...
	Float pdfDirect(const DirectSamplingRecord &dRec) const {
		...
		const Transform &trafo = m_worldTransform->eval(dRec.time);
		Spectrum temp = Spectrum(0.0f);
		Float pdfSA = internalPdfDirection(trafo.inverse()(dRec.d),temp);
	...
	void internalSampleDirection(Point2 sample, Vector &d, Spectrum &value, Float &pdf) const {
		...
		uint32_t col = sampleReuse(m_cdfCols + row * (m_size.x+1), m_size.x, sample.x);
	...
	Float internalPdfDirection(const Vector &d, Spectrum &value) const {
