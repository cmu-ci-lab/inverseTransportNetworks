	...
	switch (type) {
		...
		case ESurfaceInteraction: {
				...
				if (!(bsdf->getType() & BSDF::ENonSymmetric)) {
					...
					weight[1-mode] = Spectrum(bsdf->eval(bRec, (EMeasure) measure)[0].val()) / pdf[1-mode];
				}
				bRec.reverse();
...
bool PathVertex::perturbDirection(const Scene *scene, const PathVertex *pred,
	...
	switch (type) {
		...
		case ESurfaceInteraction: {
				...
				Spectrum value = Spectrum(bsdf->eval(bRec)[0].val());
				...
				if (!(bsdf->getType() & BSDF::ENonSymmetric)) {
					...
					weight[1-mode] = Spectrum(bsdf->eval(bRec, ESolidAngle)[0].val()) / pdf[1-mode];
				}
				bRec.reverse();
...
bool PathVertex::propagatePerturbation(const Scene *scene, const PathVertex *pred,
	...
	weight[mode] = Spectrum(bsdf->eval(bRec, EDiscrete)[0].val()) / prob;
	pdf[mode] = prob;
	measure = EDiscrete;
	...
	if (!(bsdf->getType() & BSDF::ENonSymmetric))
		...
		weight[1-mode] = Spectrum(bsdf->eval(bRec, EDiscrete)[0].val()) / pdf[1-mode];
	bRec.reverse();
...
Spectrum PathVertex::eval(const Scene *scene, const PathVertex *pred,
	...
	switch (type) {
		...
		case ESurfaceInteraction: {
				...
				result = Spectrum(bsdf->eval(bRec, measure)[0].val());
...
Spectrum PathVertex::sampleDirect(const Scene *scene, Sampler *sampler,
	...
	if (emitter)
		value = Spectrum((scene->sampleEmitterDirect(dRec, rsamp, false))[0].val());
	else
