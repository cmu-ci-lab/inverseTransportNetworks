...
public:
	 ...
	 */
	ImageBlock(int nDerivative, Bitmap::EPixelFormat fmt, const Vector2i &size,
	 ...
	 */
	FINLINE bool put(const Point2 &pos, const std::vector<Float> spec, Float alpha, int nDerivatives) {
		
		Float temp[SPECTRUM_SAMPLES + nDerivatives + 2];
		for (int i=0; i<SPECTRUM_SAMPLES + nDerivatives; i++) {
			temp[i] = spec[i];
		}
		temp[SPECTRUM_SAMPLES + nDerivatives] = alpha;
		temp[SPECTRUM_SAMPLES + nDerivatives + 1] = 1.0f;
		return put(pos, temp);
	}
	...
	FINLINE bool put(const Point2 &_pos, const Float *value) {
		...
		for (int i=0; i<channels; ++i) {
			if (EXPECT_NOT_TAKEN((!std::isfinite(value[i]) || value[0] < 0) && m_warn))
				goto bad_sample;
		}
	...
	ref<ImageBlock> clone() const {
		ref<ImageBlock> clone = new ImageBlock(3, m_bitmap->getPixelFormat(),
