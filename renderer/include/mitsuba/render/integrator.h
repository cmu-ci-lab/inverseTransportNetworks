...
public:
	 ...
	 */
	virtual Spectrum_AD Li(const RayDifferential &ray,
	...
	virtual void renderBlock(const Scene *scene, const Sensor *sensor,
		...
		const std::vector< TPoint2<uint8_t> > &points, int counter = 0) const;
	...
	/// Serialize this integrator to a binary data stream
	void serialize(Stream *stream, InstanceManager *manager) const;
	std::vector<Float> albedo_mie;
	std::vector<Float> sigmaT_mie;
	std::vector<std::vector<Float>> weights_mie;
	std::vector<Spectrum_shared> m_theta_integrator;
	std::vector<FloatAD_shared> m_theta_integrator_FloatAD;
	std::vector<FloatAD> m_theta_integrator_TextureAD;
	std::vector<std::string> m_id_AD_integrator_Spectrum_AD;
	std::vector<std::string> m_id_AD_integrator_FloatAD;
	std::vector<std::string> m_id_AD_integrator_TextureAD;
	MTS_DECLARE_CLASS()
