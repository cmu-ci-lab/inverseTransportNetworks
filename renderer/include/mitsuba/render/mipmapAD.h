...
#pragma once
#if !defined(__MITSUBA_RENDER_MIPMAPAD_H_)
#define __MITSUBA_RENDER_MIPMAPAD_H_
...
namespace stats {
	extern MTS_EXPORT_RENDER StatsCounter avgEWASamplesAD;
	extern MTS_EXPORT_RENDER StatsCounter clampedAnisotropyAD;
	extern MTS_EXPORT_RENDER StatsCounter mipStorageAD;
	extern MTS_EXPORT_RENDER StatsCounter filteredLookupsAD;
};
 ...
 * \ingroup librender
 */
template <typename Value , typename QuantizedValue> class TMIPMapAD : public Object {
public:
	...
	typedef TSpectrum_AD<FloatAD, 3> testAD;
	/**
	 ...
	 */
	TMIPMapAD(fs::path cacheFilename, Float maxAnisotropy = 20.0f)
			: m_weightLut(NULL), m_maxAnisotropy(maxAnisotropy) {
		...
		stats::mipStorageAD += m_mmap->getSize();
	...
	/// Return a bitmap representation of the given level
	ref<BitmapAD> toBitmapAD(int level = 0) const {
		const Array2DType &array = m_pyramid[level];
		ref<BitmapAD> result = new BitmapAD(
			m_pixelFormatAD,
			BitmapAD::componentFormat<typename QuantizedValue::Scalar>(),
			array.getSize()
		);
	...
	inline Value evalBox3(int level, const Point2 &uv) const {
		return evalTexel(level, uv.x, uv.y);
	}
	/**
	...
	Value eval(const Point2 &uv, const Vector2 &d0, const Vector2 &d1) const {
		...
		if (m_filterType == ETrilinear || !(minorRadius > 0) || !(majorRadius > 0) || F < 0) {
			...
			if (minorRadius * m_maxAnisotropy < majorRadius) {
				...
				++stats::clampedAnisotropyAD;
			}
			stats::clampedAnisotropyAD.incrementBase();
...
protected:
	...
	Value evalEWA(int level, const Point2 &uv, Float A, Float B, Float C) const {
		...
		if (denominator == 0) {
			...
			stats::avgEWASamplesAD += nSamples;
			stats::avgEWASamplesAD.incrementBase();
		}
...
private:
	...
	BitmapAD::EPixelFormat m_pixelFormatAD;
...
template <typename Value, typename QuantizedValue>
	Class *TMIPMapAD<Value, QuantizedValue>::m_theClass
		= new Class("MIPMap", false, "Object");

template <typename Value, typename QuantizedValue>
	const Class *TMIPMapAD<Value, QuantizedValue>::getClass() const {
	return m_theClass;
}

MTS_NAMESPACE_END

#endif /* __MITSUBA_RENDER_MIPMAPAD_H_ */
