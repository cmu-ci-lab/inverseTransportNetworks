...
public:
	 ...
	 */
	Spectrum_AD evalTransmittance(const Point &p1, bool p1OnSurface,
	 ...
	 *     between zero and one.
	 */
	Spectrum_AD evalTransmittanceAll(const Point &p1, bool p1OnSurface,
		const Point &p2, bool p2OnSurface, Float time, const Medium *medium,
		int &interactions, Sampler *sampler = NULL) const;
	 ...
	 */
	Spectrum_AD sampleEmitterDirect(DirectSamplingRecord &dRec,
	 ...
	 *    the sampled ray divided by the sample probability.
	 */
	Spectrum_AD sampleSensorDirect(DirectSamplingRecord &dRec,
			const Point2 &sample, bool testVisibility = true) const;
	 ...
	 *    the sampled ray divided by the sample probability.
	 */
	Spectrum_AD sampleAttenuatedEmitterDirect(DirectSamplingRecord &dRec,
	 ...
	 *    the sampled ray divided by the sample probability.
	 */
	Spectrum_AD sampleAttenuatedSensorDirect(DirectSamplingRecord &dRec,
			const Medium *medium, int &interactions, const Point2 &sample,
			Sampler *sampler = NULL) const;
	 ...
	 */
	inline Spectrum_AD evalEnvironment(const RayDifferential &ray) const {
		return hasEnvironmentEmitter() ?
			m_environmentEmitter->evalEnvironment(ray) : Spectrum_AD(0.0f);
	}
	 ...
	 * This is primarily meant for path tracing-style integrators.
	 */
	inline Spectrum_AD evalAttenuatedEnvironment(const RayDifferential &ray,
			const Medium *medium, Sampler *sampler) const {
		if (!m_environmentEmitter)
			return Spectrum_AD(0.0f);
		Spectrum_AD result = evalEnvironment(ray);
		if (medium)
    ...
    // void setTheta() const {
	// 	m_theta.push_back((stan::math::var) 1.0f);
	// }
	Spectrum_AD testttt;
	//! @}
	// =============================================================
...
private:
	...
	std::vector<stan::math::var> m_theta;
	Spectrum_AD testVar;
