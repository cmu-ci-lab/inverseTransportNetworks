...
public:
	...
	virtual Spectrum_AD sampleDirectAD(DirectSamplingRecord &dRec,
			const Point2 &sample) const;
			
	virtual Spectrum  evalDirect(const DirectSamplingRecord &dRec, Float &pdf) const;
	...
	virtual std::vector<ref<Shape>> createShapes(const Scene *scene);
	/**
	 * \brief Return an axis-aligned box bounding the spatial
	 * extents of the emitter
	 */
	virtual AABB getAABB() const = 0;
	virtual std::vector<AABB> getAABBs();
...
public:
	...
	virtual Spectrum_AD evalAD(const Intersection &its, const Vector &d) const;
 	/**
	...
	virtual bool isRegularEnv() const;
	 ...
	 */
	virtual Spectrum_AD evalEnvironment(const RayDifferential &ray) const;
	virtual Spectrum evalEnvironment_sky(const RayDifferential &ray) const;
	/**
