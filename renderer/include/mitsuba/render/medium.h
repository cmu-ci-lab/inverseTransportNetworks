...
public:
	 ...
	 */
	virtual Spectrum_AD evalTransmittance(const Ray &ray,
	...
	inline Spectrum_AD getSigmaA() const {
		Spectrum_AD ret;
		for ( int i = 0; i < SPECTRUM_SAMPLES; ++i ) {
			ret[i] = m_sigmaT[i].m_data[Thread::getID()] * (FloatAD(1.0f) - m_albedo[i].m_data[Thread::getID()]);
		}
		return ret;
	}

	/// For homogeneous media: return the scattering coefficient
	inline Spectrum_AD getSigmaS() const {
		Spectrum_AD ret;
		for ( int i = 0; i < SPECTRUM_SAMPLES; ++i ) {
			ret[i] = m_sigmaT[i].m_data[Thread::getID()] * m_albedo[i].m_data[Thread::getID()];
		}
		return ret;
	}

	/// For homogeneous media: return the extinction coefficient
	inline const Spectrum_AD &getSigmaT() const {
		Spectrum_AD ret;
		for ( int i = 0; i < SPECTRUM_SAMPLES; ++i ) ret[i] = m_sigmaT[i].m_data[Thread::getID()];
		return ret;
	}

	/// For homogeneous media: return the albedo
	inline const Spectrum_AD &getAlbedo() const {
		Spectrum_AD ret;
		for ( int i = 0; i < SPECTRUM_SAMPLES; ++i ) ret[i] = m_albedo[i].m_data[Thread::getID()];
	}
...
//	Spectrum_shared m_sigmaA;
//	Spectrum_shared m_sigmaS;
	Spectrum_shared m_sigmaT;
	Spectrum_shared m_albedo;
	std::vector<Float> m_phase_theta_tmp;
	std::vector<Float> m_phase_func_tmp;
	std::vector<std::vector<Float>> m_phase_func_all_tmp;
	std::vector<FloatAD_shared> m_phase_w_tmp;
	std::vector<Spectrum_shared> m_phase_w_tmp_ori;
};
