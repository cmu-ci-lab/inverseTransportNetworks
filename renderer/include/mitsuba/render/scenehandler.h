...
public:
	...
	std::vector<FloatAD> temp_TextureAD;
	std::vector<Spectrum_shared> temp_m_theta;
	std::vector<FloatAD_shared> temp_m_theta_FloatAD;	
	std::vector<std::string> temp_id_AD_FloatAD;
	std::vector<std::string> temp_id_AD_Spectrum_AD;
	std::vector<std::string> temp_id_TextureAD;
	std::vector<Float> temp_m_sigmaT_mie;
	std::vector<Float> temp_m_albedo_mie;
	std::vector<std::vector<Float>> temp_m_weights_mie;
protected:
...
private:
	...
	enum ETag {
		...
		EIntegrator, ETexture,ETextureAD, ESensor,
		EEmitter, ESubsurface, EMedium,
		EVolume, EPhase, EBSDF, ERFilter,
		ENull, EReference, EInteger, EFloat, EFloatAD,
		EBoolean, EString, ETranslate, ERotate,
		ELookAt, EScale, EMatrix, EPoint,
		EVector, EVector_AD, ERGB, ESRGB, EBlackBody,
		ESpectrum,ESpectrum_AD, ETransform, EAnimation,
