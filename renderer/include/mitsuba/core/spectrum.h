...
struct FloatAD_TEST {
    FloatAD_TEST() : m_data(128) {}

    operator stan::math::var()& {
        return m_data[mitsuba::Thread::getID()];
	}			
    std::vector<stan::math::var> m_data;
};
typedef mitsuba::TVector2<FloatAD>   Vector2AD;
typedef mitsuba::TVector3<FloatAD>   VectorAD;


//typedef stan::math::var FloatAD;
typedef std::vector<stan::math::var> Vector_AD;
...
/**
 * \brief Abstract spectral power distribution data type FOR AD version
 *
 * This class defines a vector-like data type that can be used for
 * computations involving radiance. A concrete instantiation for the
 * precision and spectral discretization chosen at compile time is
 ...
 * \ingroup libcore
 */
template <typename T, int N> struct TSpectrum_AD {
public:
	typedef T          Scalar;
	/// Number of dimensions
	const static int dim = N;
	...
	/// Create a new spectral power distribution with all samples set to the given value
	explicit inline TSpectrum_AD(Scalar v) {
		for (int i=0; i<N; i++)
			s[i] = (FloatAD) v;
	}

	/// Copy a spectral power distribution
	explicit inline TSpectrum_AD(Scalar spec[N]) {
		memcpy(s, spec, sizeof(Scalar)*N);
	}

	/// Unserialize a spectral power distribution from a binary data stream
	explicit inline TSpectrum_AD(Stream *stream) {
		stream->readArray(s, N);
	}

	/// Initialize with a TSpectrum data type based on a alternate representation
	template <typename AltScalar> explicit TSpectrum_AD(const TSpectrum_AD<AltScalar, N> &v) {
		for (int i=0; i<N; ++i) {
	...
	/// Add two spectral power distributions
	inline TSpectrum_AD operator+(const TSpectrum_AD &spec) const {
		TSpectrum_AD value = *this;
		for (int i=0; i<N; i++)
			value.s[i] += spec.s[i];
		return value;
	}
	...
	/// Add a spectral power distribution to this instance
	inline TSpectrum_AD& operator+=(const TSpectrum_AD &spec) {
		for (int i=0; i<N; i++)
			s[i] += spec.s[i];
		return *this;
	}
	...
	/// Multiply by a scalar
	inline TSpectrum_AD operator*(Scalar f) const {
		TSpectrum_AD value = *this;
		for (int i=0; i<N; i++)
			value.s[i] *= f;
		return value;
	...
	/// Multiply by a scalar
	inline friend TSpectrum_AD operator*(Scalar f, const TSpectrum_AD &spec) {
		return spec * f;
	}

	/// Multiply by a scalar
	inline TSpectrum_AD& operator*=(Scalar f) {
		for (int i=0; i<N; i++)
			s[i] *= f;
		return *this;
	}
	...
	/// Multiply by a scalar
	inline TSpectrum_AD operator*(Float f) const {
		TSpectrum_AD value = *this;
		for (int i=0; i<N; i++)
			value.s[i] *= f;
		return value;
	...
	/// Multiply by a scalar
	inline friend TSpectrum_AD operator*(Float f, const TSpectrum_AD &spec) {
		return spec * f;
	}

	/// Multiply by a scalar
	inline TSpectrum_AD& operator*=(Float f) {
		for (int i=0; i<N; i++)
			s[i] *= f;
		return *this;
	}
...
#ifdef MTS_DEBUG
		if (f == 0)
			SLog(EWarn, "TSpectrum_AD: Division by zero!");
#endif
		FloatAD recip = 1.0f / f;
		for (int i=0; i<N; i++)
			value.s[i] *= recip;
		return value;
	}
	...
	/// Equality test
	inline bool operator==(const TSpectrum_AD &spec) const {
		for (int i=0; i<N; i++) {
			if (s[i] != spec.s[i])
				return false;
		}
	...
	/// Divide by a scalar
	inline friend TSpectrum_AD operator/(Scalar f, TSpectrum_AD &spec) {
		return TSpectrum_AD(f) / spec;
	}
	...
	/// Multiply-accumulate operation, adds \a weight * \a spec
	inline void addWeighted(Scalar weight, const TSpectrum_AD &spec) {
		for (int i=0; i<N; i++)
			s[i] += weight * spec.s[i];
	}

	/// Return the average over all wavelengths
	inline FloatAD average() const {
		FloatAD result = 0.0f;
		for (int i=0; i<N; i++)
			result += s[i];
		return result * (1.0f / N);
	...
	/// Component-wise square root
	inline TSpectrum_AD sqrt() const {
		TSpectrum_AD value;
		for (int i=0; i<N; i++)
			value.s[i] = std::sqrt(s[i]);
		return value;
	...
	/// Component-wise square root
	inline TSpectrum_AD safe_sqrt() const {
		TSpectrum_AD value;
		for (int i=0; i<N; i++)
			value.s[i] = math::safe_sqrt(s[i]);
		return value;
	...
	/// Component-wise power
	inline TSpectrum_AD pow(Scalar f) const {
		TSpectrum_AD value;
		for (int i=0; i<N; i++)
			value.s[i] = std::pow(s[i], f);
		return value;
	...
	/// Return the highest-valued spectral sample
	inline FloatAD max() const {
		FloatAD result = s[0];
		for (int i=1; i<N; i++)
			result = (FloatAD)std::max(result.val(), s[i].val());
		return result;
	}

	/// Return the lowest-valued spectral sample
	inline FloatAD min() const {
		FloatAD result = s[0];
		for (int i=1; i<N; i++)
			result = (FloatAD)std::min(result.val(), s[i].val());
		return result;
	}

	/// Negate
	inline TSpectrum_AD operator-() const {
		TSpectrum_AD value;
		for (int i=0; i<N; i++)
			value.s[i] = -s[i];
		return value;
	...
	/// Indexing operator
	inline FloatAD &operator[](int entry) {
		return s[entry];
	}

	/// Indexing operator
	inline FloatAD operator[](int entry) const {
		return s[entry];
	}
 ...
 * \ingroup libpython
 */
struct MTS_EXPORT_CORE Color3AD : public TSpectrum_AD<FloatAD, 3> {
public:
	typedef TSpectrum_AD<FloatAD, 3> Parent;
	...
	inline Color3AD() {
		for (int i=0; i<3; i++)
			s[i] = std::numeric_limits<FloatAD>::quiet_NaN();
	}
#endif

	/// Copy constructor
	inline Color3AD(const Parent &s) : Parent(s) { }

	/// Initialize to a constant value
	inline Color3AD(FloatAD value) : Parent(value) { }

	/// Initialize to the given RGB value
	inline Color3AD(FloatAD r, FloatAD g, FloatAD b) {
		s[0] = r; s[1] = g; s[2] = b;
	}

	/// Return the luminance (assuming the color value is expressed in linear sRGB)
	inline FloatAD getLuminance() const {
		return s[0] * 0.212671f + s[1] * 0.715160f + s[2] * 0.072169f;
	}
};

struct MTS_EXPORT_CORE Spectrum_AD : public TSpectrum_AD<FloatAD, SPECTRUM_SAMPLES> {
public:
	typedef TSpectrum_AD<FloatAD, SPECTRUM_SAMPLES> Parent;
	...
	/// Construct from a TSpectrum instance
	inline Spectrum_AD(const Parent &s) : Parent(s) { }
	/// Initialize with a TSpectrum data type based on a alternate representation
	template <typename AltScalar> explicit Spectrum_AD(const TSpectrum_AD<AltScalar, SPECTRUM_SAMPLES> &v) {
		for (int i=0; i<SPECTRUM_SAMPLES; ++i) {
			Scalar temp = (Scalar) v[i];
			s[i] = (FloatAD) temp;
		}
	...
	/// Create a new spectral power distribution with all samples set to the given value
	explicit inline Spectrum_AD(FloatAD v) {
		for (int i=0; i<SPECTRUM_SAMPLES; i++)
			s[i] = v;
	}
	explicit inline Spectrum_AD(Spectrum v) {
		for (int i=0; i<SPECTRUM_SAMPLES; i++)
			s[i] = v[i];
	}
...
#if SPECTRUM_SAMPLES == 3
	inline void fromLinearRGB(FloatAD r, FloatAD g, FloatAD b,
		EConversionIntent intent = EReflectance /* unused */) {
	/* Nothing to do -- the renderer is in RGB mode */
	s[0] = r; s[1] = g; s[2] = b;
	}
	...
	void fromLinearRGB(FloatAD r, FloatAD g, FloatAD b,
			EConversionIntent intent = EReflectance);
#endif


	Spectrum_AD& operator*=(const Spectrum &spec) {
			s[0] *= spec[0];
			return *this;
	}
	...
	Spectrum_AD& operator*=(const Float a) {
			s[0] *= a;
			return *this;
	}
	...
	Spectrum_AD& operator+=(const Spectrum_AD &spec){
			s[0] += spec[0];
			return *this;
	}	
	Spectrum_AD& operator-=(const Spectrum_AD &spec){
	...
	/// Copy a spectral power distribution
	explicit inline Spectrum_AD(FloatAD value[SPECTRUM_SAMPLES]) {
		memcpy(s, value, sizeof(FloatAD)*SPECTRUM_SAMPLES);
	}

	/// Unserialize a spectral power distribution from a binary data stream
	explicit inline Spectrum_AD(Stream *stream) : Parent(stream) { }

	/**
	 * \brief Evaluate the SPD for the given wavelength
	 * in nanometers.
	 */
	FloatAD eval(FloatAD lambda) const;
...
inline Spectrum_AD operator+(Spectrum_AD x, Spectrum y)  {
	for (int i=0; i<SPECTRUM_SAMPLES; i++)
		x[i] += y[i];
	return x;
}
...
inline Spectrum_AD operator*(Spectrum_AD x, float y)  {
	for (int i=0; i<SPECTRUM_SAMPLES; i++)
		x[i] *= y;
	return x;
}
...
inline Spectrum_AD operator+(Spectrum_AD x, FloatAD y)  {
	for (int i=0; i<SPECTRUM_SAMPLES; i++)
		x[i] += y;
	return x;
}
...
inline FloatAD getMin(FloatAD x, Float y) {
	if (x.val() <= y) {
		return x;
	} else {
		return y;
