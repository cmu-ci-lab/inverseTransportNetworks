...
#pragma once
#if !defined(__MITSUBA_CORE_BITMAPAD_H_)
#define __MITSUBA_CORE_BITMAPAD_H_
 ...
 * \ingroup libpython
 */
class MTS_EXPORT_CORE BitmapAD : public Object {
public:
	// ======================================================================
	 ...
	 */
	BitmapAD(EPixelFormat pFmt, EComponentFormat cFmt, const Vector2i &size,
		uint8_t channelCount = 0, int nDerivative = 0, uint8_t *data = NULL);
	 ...
	 */
	BitmapAD(EFileFormat format, Stream *stream, const std::string &prefix = "");
	 ...
	 *    This is currently only supported by the OpenEXR format loader.
	 */
	BitmapAD(const fs::path &path, const std::string &prefix = "");
	...
	bool operator==(const BitmapAD &bitmaAD) const;

	/// Bitmap inequality operator (useful for unit-tests etc.)
	inline bool operator!=(const BitmapAD &bitmapAD) const
		{ return !operator==(bitmapAD); }

	/// Create an identical copy
	ref<BitmapAD> clone() const;
	 ...
	 *      should be resolved.
	 */
	void convert(BitmapAD *target, Float multiplier = 1.0f,
	 ...
	 *      should be resolved.
	 */
	ref<BitmapAD> convert(EPixelFormat pixelFormat, EComponentFormat componentFormat,
	 ...
	 */
	ref<BitmapAD> convertMultiSpectrumAlphaWeight(const std::vector<EPixelFormat> &pixelFormats,
	...
	static void convertMultiSpectrumAlphaWeight(const BitmapAD *source,
	 ...
	 */
	ref<BitmapAD> extractChannel(int channelIndex) const;

	/**
	 * \brief Extract several color channels of a multi-channel
	 * bitmap and return them as a bitmap with the given pixel format
	 */
	ref<BitmapAD> extractChannels(BitmapAD::EPixelFormat fmt, const std::vector<int> &channels) const;
	 ...
	 */
	std::map<std::string, BitmapAD *> split() const;
	 ...
	 * as color channels
	 */
	static ref<BitmapAD> join(EPixelFormat fmt,
			const std::vector<BitmapAD *> &sourceBitmaps);

	/// Crop the bitmap to the specified rectangle
	ref<BitmapAD> crop(const Point2i &offset, const Vector2i &size) const;
	...
	ref<BitmapAD> rotateFlip(ERotateFlipType type) const;
	 ...
	 */
	void copyFrom(const BitmapAD *bitmapAD, Point2i sourceOffset,
	 ...
	 * component format is \ref EBitmask.
	 */
	inline void copyFrom(const BitmapAD *bitmapAD, Point2i targetOffset) {
		copyFrom(bitmapAD, Point2i(0), targetOffset, bitmapAD->getSize());
	}
	 ...
	 * component format is \ref EBitmask.
	 */
	inline void copyFrom(const BitmapAD *bitmapAD) {
		copyFrom(bitmapAD, Point2i(0), Point2i(0), bitmapAD->getSize());
	}
	 ...
	 * component format is \ref EBitmask.
	 */
	void accumulate(const BitmapAD *bitmapAD, Point2i sourceOffset,
			Point2i targetOffset, Vector2i size);
	 ...
	 * component format is \ref EBitmask.
	 */
	inline void accumulate(const BitmapAD *bitmapAD, Point2i targetOffset) {
		accumulate(bitmapAD, Point2i(0), targetOffset, bitmapAD->getSize());
	}
	 ...
	 * component format is \ref EBitmask.
	 */
	inline void accumulate(const BitmapAD *bitmapAD) {
		accumulate(bitmapAD, Point2i(0), Point2i(0), bitmapAD->getSize());
	}
	 ...
	 */
	void convolve(const BitmapAD *kernel);
	 ...
	 */
	static ref<BitmapAD> arithmeticOperation(EArithmeticOperation operation,
			const BitmapAD *bitmap1, const BitmapAD *bitmap2);
	...
	void resample(const ReconstructionFilter *rfilter,
		...
		BitmapAD *target, BitmapAD *temp = NULL,
	 ...
	 */
	ref<BitmapAD> resample(const ReconstructionFilter *rfilter,
		ReconstructionFilter::EBoundaryCondition bch,
		ReconstructionFilter::EBoundaryCondition bcv,
	 ...
	 * This function allocates a new output image and returns it.
	 */
	ref<BitmapAD> filter(const ReconstructionFilter *rfilter,
		ReconstructionFilter::EBoundaryCondition bch,
		ReconstructionFilter::EBoundaryCondition bcv,
		Float minValue = 0.0f, Float maxValue = 1.0f) const;
 ...
 * \ingroup libcore
 */
class MTS_EXPORT_CORE FormatConverterAD {
public:
	typedef BitmapAD::EComponentFormat                Format;
	typedef std::pair<Format, Format>               Conversion;
	typedef std::map<Conversion, FormatConverterAD *> ConverterMap;
	...
	virtual void convert(
			BitmapAD::EPixelFormat sourceFormat, Float sourceGamma, const void *_source,
			BitmapAD::EPixelFormat destFormat, Float destGamma, void *_dest,
			size_t count, Float multiplier = 1.0f,
			Spectrum::EConversionIntent intent = Spectrum::EReflectance,
			int channelCount = -1, int nDerivative = 1) const= 0;
...
//! \cond
namespace detail {
	template <typename T> inline BitmapAD::EComponentFormat cfmt() { return BitmapAD::EInvalid; }
	template <> inline BitmapAD::EComponentFormat cfmt<uint8_t>() { return BitmapAD::EUInt8; }
	template <> inline BitmapAD::EComponentFormat cfmt<uint16_t>() { return BitmapAD::EUInt16; }
	template <> inline BitmapAD::EComponentFormat cfmt<uint32_t>() { return BitmapAD::EUInt32; }
	template <> inline BitmapAD::EComponentFormat cfmt<half>() { return BitmapAD::EFloat16; }
	template <> inline BitmapAD::EComponentFormat cfmt<float>() { return BitmapAD::EFloat32; }
	template <> inline BitmapAD::EComponentFormat cfmt<double>() { return BitmapAD::EFloat64; }
};

template <typename T> inline BitmapAD::EComponentFormat BitmapAD::componentFormat() {
	return detail::cfmt<T>();
}

extern MTS_EXPORT_CORE std::ostream &operator<<(std::ostream &os, const BitmapAD::EPixelFormat &value);
extern MTS_EXPORT_CORE std::ostream &operator<<(std::ostream &os, const BitmapAD::EComponentFormat &value);
//! \endcond

MTS_NAMESPACE_END

#endif /* __MITSUBA_CORE_BITMAPAD_H_ */
