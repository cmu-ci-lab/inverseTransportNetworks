...
public:
	...
	/// Set a single precision floating point value
	void setFloatAD(const std::string &name, const FloatAD &value, bool warnDuplicates = true);
	/// Get a single precision floating point value
	FloatAD getFloatAD(const std::string &name) const;
	/// Get a single precision floating point value (with default)
	FloatAD getFloatAD(const std::string &name, const FloatAD &defVal) const;
	...
	Spectrum getSpectrum(const std::string &name, const Spectrum &defVal) const;
		...
		/// Set a spectral power distribution
	void setSpectrum_AD(const std::string &name, const Spectrum_AD &value, bool warnDuplicates = true);
	/// Get a spectral power distribution
	Spectrum_AD getSpectrum_AD(const std::string &name) const;
	/// Get a spectral power distribution (with default)
	Spectrum_AD getSpectrum_AD(const std::string &name, const Spectrum_AD &defVal) const;
	/// Set a 3d point
	...
	/// Set a 3d vector
	void setVector_AD(const std::string &name, const Vector_AD &value, bool warnDuplicates = true);
	/// Get a 3d vector
	Vector_AD getVector_AD(const std::string &name) const;
	/// Get a 3d vector (with default)
	Vector_AD getVector_AD(const std::string &name, const Vector_AD &defVal) const;
	...
	std::vector<Spectrum_shared> m_theta;	
	std::vector<FloatAD_shared> m_theta_FloatAD;
	std::vector<FloatAD> m_theta_TextureAD;
	std::vector<std::string> id_AD_TextureAD;	
	std::vector<std::string> id_AD_Spectrum_AD;
	std::vector<std::string> id_AD_FloatAD;			
	std::vector<std::string> m_theta_string;
	std::vector<Float> m_phase_theta;
	std::vector<Float> m_phase_func;
	std::vector<FloatAD_shared> m_phase_w;
	std::vector<Spectrum_shared> m_phase_orig;
	std::vector<std::vector<Float>> m_phase_func_all;
	std::vector<Float> m_sigmaT_mie;
	std::vector<Float> m_albedo_mie;
	std::vector<std::vector<Float>> m_weights_mie;

	bool checkMixture = false;	
private:
