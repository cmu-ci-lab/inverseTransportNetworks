  ...
  namespace math {
    ...
    public:
      ...
      explicit vari(double x):
        val_(x),
        adj_(0.0) {
        mitsuba::StanStack::getInstance()->var_stack_.push_back(this);
      }

      explicit vari(double x, int tid):
        val_(x),
        adj_(0.0) {
        mitsuba::StanStack::getInstance(tid)->var_stack_.push_back(this);
      }
      ...
      vari(double x, bool stacked):
        ...
        if (stacked)
          ...
          mitsuba::StanStack::getInstance()->var_nochain_stack_.push_back(this);
      }

      vari(double x, bool stacked, int tid):
        val_(x),
        adj_(0.0) {
        if (stacked)
          mitsuba::StanStack::getInstance(tid)->var_stack_.push_back(this);
        else
          mitsuba::StanStack::getInstance(tid)->var_nochain_stack_.push_back(this);
      }
      ...
      static inline void* operator new(size_t nbytes) {
        return mitsuba::StanStack::getInstance()->memalloc_.alloc(nbytes);
      }
