    ...
    public:
      ...
      var build(double value) {
        size_t size = edge1_.size() + edge2_.size() + edge3_.size()
          + edge4_.size();
        vari** varis = mitsuba::StanStack::getInstance()->memalloc_.alloc_array<vari*>(size);
        double* partials = mitsuba::StanStack::getInstance()->memalloc_.alloc_array<double>(size);
        int idx = 0;
