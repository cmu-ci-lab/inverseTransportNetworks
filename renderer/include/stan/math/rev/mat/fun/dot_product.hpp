    ...
    namespace {
      ...
      protected:
        ...
        inline void initialize(vari** &mem_v, const var *inv,
                               vari **shared = NULL) {
          if (shared == NULL) {
            mem_v = reinterpret_cast<vari**>(mitsuba::StanStack::getInstance()->memalloc_
        ...
        inline void initialize(vari** &mem_v,
        ...
        inline void initialize(double* &mem_d, const double *ind,
                               double *shared = NULL) {
          if (shared == NULL) {
            mem_d = reinterpret_cast<double*>(mitsuba::StanStack::getInstance()->memalloc_
        ...
        template<typename Derived>
        inline void initialize(double* &mem_d,
                               const Eigen::DenseBase<Derived> &ind,
                               double *shared = NULL) {
          if (shared == NULL) {
            mem_d = reinterpret_cast<double*>
              (mitsuba::StanStack::getInstance()->memalloc_.alloc(length_*sizeof(double)));
            for (size_t i = 0; i < length_; i++)
              mem_d[i] = ind(i);
          } else {
            mem_d = shared;
