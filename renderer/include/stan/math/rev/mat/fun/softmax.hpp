...
namespace stan {
  namespace math {
    ...
    softmax(const Eigen::Matrix<var, Eigen::Dynamic, 1>& alpha) {
      ...
      vari** alpha_vi_array
		= reinterpret_cast<vari**>(mitsuba::StanStack::getInstance()->memalloc_		
      ...
      double* softmax_alpha_d_array
		= reinterpret_cast<double*>(mitsuba::StanStack::getInstance()->memalloc_
                                    .alloc(sizeof(double) * alpha_d.size()));
      for (int i = 0; i < alpha_d.size(); ++i)
