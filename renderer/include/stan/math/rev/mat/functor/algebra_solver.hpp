  ...
  namespace math {
    ...
    struct algebra_solver_vari : public vari {
      ...
      algebra_solver_vari(const FS& fs,
                          ...
                          std::ostream* msgs)
        : vari(theta_dbl(0)),
          y_(mitsuba::StanStack::getInstance()->memalloc_.alloc_array<vari*>(y.size())),
          y_size_(y.size()),
          x_size_(x.size()),
          theta_(mitsuba::StanStack::getInstance()->memalloc_.alloc_array<vari*>(x_size_)),
          Jx_y_(mitsuba::StanStack::getInstance()->memalloc_.alloc_array<double>
                  (x_size_ * y_size_)) {
        ...
        Map<MatrixXd>(&Jx_y_[0], x_size_, y_size_)
          = - mdivide_left(fx.get_jacobian(theta_dbl),
                           f_y(fs, f, theta_dbl, value_of(y),
                               dat, dat_int, msgs).get_jacobian(value_of(y)));
      }
    ...
    algebra_solver(const F& f,
      ...
      check_matching_sizes("algebra_solver",
                           "the algebraic system's output",
                           fx.get_value(x),
