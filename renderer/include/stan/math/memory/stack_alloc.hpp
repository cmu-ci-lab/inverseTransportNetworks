...
namespace stan {
  namespace math {
    ...
    private:
      ...
      size_t blocks_record_;
      size_t sizes_record_;
      size_t cur_block_record_;
      char* cur_block_end_record_;
      char* next_loc_record_;
      size_t nested_cur_blocks_record_;
      size_t nested_next_locs_record_;
      size_t nested_cur_block_ends_record_;
    ...
    public:
      ...
      explicit stack_alloc(size_t initial_nbytes = DEFAULT_INITIAL_NBYTES) :
        ...
        sizes_record_(0),
        cur_block_record_(0),
        cur_block_end_record_(0),
        next_loc_record_(0),
        nested_cur_blocks_record_(0),
        nested_next_locs_record_(0),
        nested_cur_block_ends_record_(0) {
        if (!blocks_[0])
      ...
      inline void record() {
    	  blocks_record_ = blocks_.size();
		  sizes_record_ = sizes_.size();
		  cur_block_record_ = cur_block_;
		  cur_block_end_record_ = cur_block_end_;
		  next_loc_record_ = next_loc_;
		  nested_cur_blocks_record_ = nested_cur_blocks_.size();
		  nested_next_locs_record_ = nested_next_locs_.size();
		  nested_cur_block_ends_record_ = nested_cur_block_ends_.size();
      }

      inline void revert() {
		 // frees all BUT the first (index 0) block
		 for (size_t i = blocks_record_; i < blocks_.size(); ++i)
		   if (blocks_[i])
			 free(blocks_[i]);

		 blocks_.resize(blocks_record_);
		 sizes_.resize(sizes_record_);
		 cur_block_ = cur_block_record_;
		 cur_block_end_ = cur_block_end_record_;
		 next_loc_ = next_loc_record_;
		 nested_cur_blocks_.resize(nested_cur_blocks_record_);
		 nested_next_locs_.resize(nested_next_locs_record_);
		 nested_cur_block_ends_.resize(nested_cur_block_ends_record_);
      }
